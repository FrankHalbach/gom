...
export const calcYOYReport = (forecast: Variant, actual: Variant): GOMReport => {
    ...
    const factor = (): number | null => {
        ...
        const actPartVolume: number = report.actualPartVolume ?? 0
        const vehActVolume: number = report.vehicles.map(v => v.actualPartVolume).reduce((a, b) => a + b, 0)

        if (vehActVolume == 0) return 1

        return actPartVolume / vehActVolume
